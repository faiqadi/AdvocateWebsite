...
export default async function ContactPage() {
  ...
  const primaryContact = contacts[0]; // Use first contact as primary
  return (
    <div
          ...
          <div className="max-w-5xl mx-auto">
            <ScrollAnimation direction="up">
              <div className="relative group p-8 bg-white/80 dark:bg-slate-900/60 border border-slate-200 dark:border-white/10 hover:border-accent transition-all duration-300 shadow-sm hover:shadow-xl backdrop-blur-sm mb-12">
                ...
                <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-8 text-center uppercase tracking-wide">
                  {primaryContact ? `${primaryContact.city.toUpperCase()} OFFICE` : 'OFFICE'}
                </h2>
                ...
                <div className="w-full h-96 lg:h-[500px] border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900 mb-8 p-1">
                  {primaryContact?.mapsEmbedUrl ? (
                    <iframe
                      src={primaryContact.mapsEmbedUrl}
                      width="100%"
                      height="100%"
                      style={{ border: 0 }}
                      allowFullScreen
                      ...
                      title={`${primaryContact.officeName} Office`}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 border-t border-slate-200 dark:border-white/5 pt-8">
                    ...
                    {primaryContact && (
                      <>
                        <p className="text-slate-800 dark:text-slate-200 text-lg font-bold mb-2">
                          {primaryContact.buildingName || primaryContact.officeName}
                        </p>
                        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
                          {primaryContact.address}
                          <br />
                          {primaryContact.city}, {primaryContact.province} {primaryContact.postalCode}
                        </p>
                        {primaryContact.mapsUrl && (
                          <div className="mt-4">
                            <Link
                              href={primaryContact.mapsUrl}
                              target="_blank"
                  ...
                  <div className="text-center md:text-right">
                    ...
                    <ul className="space-y-4">
                      {primaryContact?.phone && String(primaryContact.phone).trim() && (
                        <li>
                          <span className="block text-xs text-slate-500 uppercase">Phone</span>
                          <a href={`tel:${String(primaryContact.phone).replace(/\D/g, '')}`} className="text-slate-900 dark:text-white font-mono hover:text-accent transition-colors">
                      ...
                      {primaryContact?.email && (
                        <li>
                          ...
                          <a href={`mailto:${primaryContact.email}`} className="text-slate-900 dark:text-white font-mono hover:text-accent transition-colors">
                            {primaryContact.email}
                          </a>
                        </li>
                      )}
            ...
            <ScrollAnimation direction="up" delay={200}>
              <div className="inline-block p-6 border border-slate-200 dark:border-white/5 bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm">
                ...
                <div className="flex justify-center items-center space-x-8">
                  {primaryContact?.socialLinks?.linkedin && (
                    <Link
                      href={primaryContact.socialLinks.linkedin}
                      target="_blank"
                      rel="noopener noreferrer"
                  ...
                  {primaryContact?.socialLinks?.instagram && (
                    <Link
                      href={primaryContact.socialLinks.instagram}
                      target="_blank"
                      rel="noopener noreferrer"
                  ...
                  {(!primaryContact?.socialLinks?.linkedin && !primaryContact?.socialLinks?.instagram) && (
                    <>
                      {/* Default social links if no data */}
                      <Link
                        href="#"
                        className="text-slate-400 hover:text-[#0077b5] transition-colors transform hover:scale-110 duration-300"
                        aria-label="LinkedIn"
