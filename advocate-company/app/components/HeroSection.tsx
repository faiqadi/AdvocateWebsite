...
export default function HeroSection() {
  ...
  return (
    <div className="relative h-screen min-h-[700px] flex flex-col md:flex-row overflow-hidden bg-slate-950">

      {/* Left Column: Content */}
      <div className="relative w-full md:w-1/2 h-full flex flex-col justify-center px-8 md:px-16 lg:px-24 z-20 bg-slate-950/90 md:bg-slate-950">
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          ...
          <div className="absolute bottom-12 right-0 w-64 h-64 border border-slate-800/20 rounded-full opacity-20"></div>
        </div>
        ...
        <div className="relative min-h-[400px] flex items-center">
          {heroSlides.map((slide, index) => (
            ...
            >
              ...
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 text-white leading-tight uppercase font-sans tracking-tight">
                {slide.title}
              </h1>

              <p className="text-lg md:text-xl text-slate-400 font-light mb-8 max-w-lg border-l-4 border-slate-800 pl-4 py-1">
                {slide.subtitle}
              </p>

              {slide.description && (
                <p className="text-sm md:text-base text-slate-500 mb-8 max-w-lg leading-relaxed hidden lg:block">
                  {slide.description}
                </p>
              )}
              ...
              <Link
                href="/contact"
                className="group inline-flex items-center gap-4 text-white font-bold uppercase tracking-[0.2em] text-sm hover:text-accent transition-colors"
              >
        ...
        <div className="absolute bottom-8 left-8 md:left-24 flex items-center gap-4">
          <button
            ...
            className="p-2 border border-slate-800 text-slate-500 hover:text-white hover:border-accent transition-colors rotate-180"
          >
            →
          </button>
            ...
            {heroSlides.map((_, index) => (
              <button
                ...
                className={`h-1.5 transition-all duration-300 ${index === currentSlide
                    ? 'bg-accent w-8'
                    : 'bg-slate-800 w-4 hover:bg-slate-700'
                  }`}
          ...
          <button
            ...
            className="p-2 border border-slate-800 text-slate-500 hover:text-white hover:border-accent transition-colors"
          >
            →
          </button>
        </div>
      ...
      <div className="absolute inset-0 md:relative md:w-1/2 h-full z-10 md:z-auto">
        {heroSlides.map((_, index) => (
          ...
          >
            ...
            <div className="absolute inset-0 bg-slate-950/80 md:bg-slate-950/30 md:mix-blend-multiply transition-colors duration-500"></div>
        ...
        <div className="absolute bottom-10 right-10 hidden md:block z-20">
          <div className="text-right">
            <div className="text-4xl font-bold text-white/10 font-mono">0{currentSlide + 1}</div>
            <div className="text-xs text-white/20 font-mono tracking-widest">CURRENT DISPLAY</div>
          </div>
        </div>
      </div>
